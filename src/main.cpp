...
void loop() {
  ...
  if(WiFi.status()== WL_CONNECTED){
    ...
    String url = "https://" + String(host) + "/api/state HTTP/1.1";
    http.begin(url);
    ...
    for(int i=0; i<8; i++){
      ...
      digitalWrite(switch_1,state[i][0]);
      digitalWrite(switch_2,state[i][1]);
      digitalWrite(switch_3,state[i][2]);


      doc["light_switch_1"] = state[i][0];
      doc["light_switch_2"] = state[i][1];
      doc["light_switch_3"] = state[i][2];
